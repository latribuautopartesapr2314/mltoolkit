...
POSIBLES_SKU = ["sku", "SKU", "Sku", "item_code", "ITEM_CODE", "Item Code", "codigo", "Código", "CODIGO"]
POSIBLES_STOCK_ORIGEN = ["AStk", "astk", "ASTK", "stock", "Stock", "STOCK"]
POSIBLES_STOCK_DESTINO = ["cantidad", "Cantidad", "CANTIDAD", "stock", "Stock", "STOCK"]
POSIBLES_PRECIO_DESTINO = ["precio", "Precio", "PRECIO", "price", "Price", "PRICE"]
POSIBLES_ESTADO_DESTINO = ["estado", "Estado", "ESTADO", "status", "Status", "STATUS"]
POSIBLES_CUOTAS = ["cuotas", "Cuotas", "CUOTAS"]
POSIBLES_MLA = ["mla", "MLA", "Mla", "id", "ID", "publicacion", "Publicacion", "PUBLICACION"]

COLUMNAS_PRECIO_ORIGEN = {
    "sin_cuotas": "Precio ML Clasica_2",
    "3_cuotas": "Precio ML Premium_2",
    "6_cuotas": "Precio ML Premium 6c_2",
    "9_cuotas": "Precio ML Premium 9c_2",
    "12_cuotas": "Precio ML Premium 12c_2",
}
...
def detectar_columna(columnas, posibles, obligatorio=True, descripcion="columna"):
    ...
    for posible in posibles:
        if posible in columnas_lista:
            return posible
    if obligatorio:
        raise ValueError(f"No se encontro {descripcion}. Columnas disponibles: {', '.join(map(str, columnas_lista))}")
    return None
...
def convertir_precio_a_numero(valor):
    ...
    # Formato argentino: 1.500.000,00
...
def clasificar_cuotas(valor):
    ...
    if "no agregar" in texto or "sin cuota" in texto:
        return "sin_cuotas"
...
def determinar_estado(precio_final, stock_final):
    ...
    try:
        ...
        return None, "Precio final invalido"
...
def armar_base_actualizacion(actualizacion_bytes):
    ...
    for hoja in xls.sheet_names:
        ...
        df = pd.read_excel(BytesIO(actualizacion_bytes), sheet_name=hoja, dtype=str)
        if df.empty:
            control_hojas_ignoradas.append({"Hoja": hoja, "Motivo": "Hoja vacia"})
            continue

        columnas = list(df.columns)
        try:
            col_sku = detectar_columna(columnas, POSIBLES_SKU, True, "columna SKU origen")
            col_stock = detectar_columna(columnas, POSIBLES_STOCK_ORIGEN, True, "columna stock origen")
        except Exception as e:
            control_hojas_ignoradas.append({"Hoja": hoja, "Motivo": str(e)})
            continue

        faltan_precios = [col for col in COLUMNAS_PRECIO_ORIGEN.values() if col not in columnas]
        if faltan_precios:
            control_hojas_ignoradas.append({"Hoja": hoja, "Motivo": "Faltan columnas de precio: " + ", ".join(faltan_precios)})
            continue
        ...
        for idx, fila in df.iterrows():
            ...
            for clave_precio, col_precio in COLUMNAS_PRECIO_ORIGEN.items():
                precio = formatear_numero(convertir_precio_a_numero(fila[col_precio]))
                registro[clave_precio] = precio
                if precio is None:
    ...
    if not filas_origen:
        raise ValueError("No se pudo armar la base de actualizacion. Revisar SKU, AStk y columnas de precio *_2.")

    df_origen = pd.DataFrame(filas_origen)
    duplicados = df_origen[df_origen["_SKU_KEY"].duplicated(keep=False)].copy()
    df_unico = df_origen.drop_duplicates(subset="_SKU_KEY", keep="first").copy()
    datos_por_sku = df_unico.set_index("_SKU_KEY").to_dict(orient="index")

    return datos_por_sku, duplicados, control_hojas_ignoradas, control_precios_invalidos, control_stock_invalido


def procesar_archivos(integraly_bytes, actualizacion_bytes):
    datos_por_sku, duplicados, hojas_ignoradas, precios_invalidos, stock_invalido = armar_base_actualizacion(actualizacion_bytes)
    ...
    for ws in wb.worksheets:
        ...
        col_sku_nombre = detectar_columna(headers.keys(), POSIBLES_SKU, obligatorio=False, descripcion="columna SKU destino")
        if col_sku_nombre is None:
            control_resumen.append({
                "Hoja destino": ws.title,
                "Estado hoja": "Ignorada",
                "Motivo": "No se encontro columna SKU",
                "Filas procesadas": 0,
        ...
        col_mla_nombre = detectar_columna(headers.keys(), POSIBLES_MLA, False, "columna MLA")
        col_cuotas_nombre = detectar_columna(headers.keys(), POSIBLES_CUOTAS, False, "columna cuotas")
        col_precio_nombre = detectar_columna(headers.keys(), POSIBLES_PRECIO_DESTINO, False, "columna precio destino")
        col_stock_nombre = detectar_columna(headers.keys(), POSIBLES_STOCK_DESTINO, False, "columna stock destino")
        col_estado_nombre = detectar_columna(headers.keys(), POSIBLES_ESTADO_DESTINO, False, "columna estado destino")
    ...
    ws_control.append(["Condicion en Integraly", "Columna usada en actualizacion precio"])
    ws_control.append(["Sin cuotas / No Agregar Cuotas / vacio", "Precio ML Clasica_2"])
    ws_control.append(["3 cuotas", "Precio ML Premium_2"])
    ws_control.append(["6 cuotas", "Precio ML Premium 6c_2"])
    ws_control.append(["9 cuotas", "Precio ML Premium 9c_2"])
    ws_control.append(["12 cuotas", "Precio ML Premium 12c_2"])
    ws_control.append(["Stock", "AStk"])
    ws_control.append([])

    ws_control.append(["REGLA DE ESTADO"])
    ws_control.append(["Condicion", "Estado asignado"])
    ...
    resumen_cols = [
        "Hoja destino", "Estado hoja", "Motivo", "Filas procesadas", "Filas actualizadas con precio",
        "SKU sin match", "Cuotas no reconocidas", "Precio no actualizado", "Stock no actualizado",
        "Estado actualizado", "Estado no actualizado", "Pausadas por superar 1.500.000"
    ]
    ws_control.append(resumen_cols)
    for r in control_resumen:
        ws_control.append([r.get(c, "") for c in resumen_cols])

    agregar_tabla(ws_control, "SKU SIN MATCH", ["Hoja destino", "Fila destino", "MLA", "SKU", "Cuotas", "Estado anterior"], control_sku_sin_match, ["Sin SKU sin match", "", "", "", "", ""])
    agregar_tabla(ws_control, "CUOTAS NO RECONOCIDAS", ["Hoja destino", "Fila destino", "MLA", "SKU", "Cuotas"], control_cuotas_no_reconocidas, ["Sin cuotas no reconocidas", "", "", "", ""])
    agregar_tabla(ws_control, "PRECIO NO ACTUALIZADO", ["Hoja destino", "Fila destino", "MLA", "SKU", "Cuotas", "Precio requerido", "Hoja origen", "Fila origen"], control_precio_no_actualizado, ["Sin precios pendientes", "", "", "", "", "", "", ""])
    agregar_tabla(ws_control, "STOCK NO ACTUALIZADO", ["Hoja destino", "Fila destino", "MLA", "SKU", "Valor AStk", "Hoja origen", "Fila origen"], control_stock_no_actualizado, ["Sin stocks pendientes", "", "", "", "", "", ""])
    agregar_tabla(ws_control, "ESTADO ACTUALIZADO", ["Hoja destino", "Fila destino", "MLA", "SKU", "Precio final", "Stock final", "Estado anterior", "Estado final", "Motivo"], control_estado_actualizado, ["Sin estados actualizados", "", "", "", "", "", "", "", ""])
    agregar_tabla(ws_control, "ESTADO NO ACTUALIZADO", ["Hoja destino", "Fila destino", "MLA", "SKU", "Precio final", "Stock final", "Estado anterior", "Motivo"], control_estado_no_actualizado, ["Sin estados pendientes", "", "", "", "", "", "", ""])
    agregar_tabla(ws_control, "PAUSADAS POR SUPERAR 1.500.000", ["Hoja destino", "Fila destino", "MLA", "SKU", "Precio final", "Stock final", "Estado anterior", "Estado final", "Motivo"], control_mayor_umbral_pausada, ["Sin publicaciones mayores a 1.500.000 pausadas", "", "", "", "", "", "", "", ""])

    ws_control.append([])
    ws_control.append(["SKU DUPLICADOS EN ACTUALIZACION PRECIO"])
    ws_control.append(["SKU", "Cantidad apariciones", "Hojas origen"])
    if not duplicados.empty:
        dup_resumen = duplicados.groupby("_SKU_KEY").agg(
        ...
        for _, r in dup_resumen.iterrows():
            ws_control.append([r["_SKU_KEY"], int(r["cantidad_apariciones"]), r["hojas_origen"]])
    else:
        ws_control.append(["Sin duplicados", 0, ""])

    agregar_tabla(ws_control, "HOJAS IGNORADAS EN ACTUALIZACION PRECIO", ["Hoja", "Motivo"], hojas_ignoradas, ["Sin hojas ignoradas", ""])
    agregar_tabla(ws_control, "PRECIOS INVALIDOS EN ACTUALIZACION PRECIO", ["Hoja origen", "Fila origen", "SKU", "Columna precio", "Valor original"], precios_invalidos, ["Sin precios invalidos", "", "", "", ""])
    agregar_tabla(ws_control, "STOCK INVALIDO EN ACTUALIZACION PRECIO", ["Hoja origen", "Fila origen", "SKU", "Valor AStk"], stock_invalido, ["Sin stocks invalidos", "", "", ""])
...
# INTERFAZ STREAMLIT
# ============================================================
st.set_page_config(page_title="Cruce Integraly - Actualizacion Precio", page_icon="📦", layout="wide")
st.title("Cruce Integraly vs Actualizacion Precio")
st.caption("Actualiza precio, stock y estado por SKU.")

st.warning(
    "Regla aplicada: precio mayor a $1.500.000 queda Pausada siempre. "
    "Precio menor o igual a $1.500.000 con stock >= 1 queda Activa. "
    "Precio menor o igual a $1.500.000 con stock <= 0 queda Pausada."
)

col1, col2 = st.columns(2)
with col1:
    archivo_integraly = st.file_uploader("Subi el archivo Integraly", type=["xlsx"], key="integraly")
with col2:
    archivo_actualizacion = st.file_uploader("Subi el archivo Actualizacion Precio", type=["xlsx"], key="actualizacion")

if st.button("Procesar cruce", type="primary"):
    if archivo_integraly is None or archivo_actualizacion is None:
        st.error("Tenes que subir los 2 archivos para procesar.")
    else:
        try:
            with st.spinner("Procesando archivos..."):
                salida, resumen = procesar_archivos(archivo_integraly.getvalue(), archivo_actualizacion.getvalue())

            st.success("Proceso finalizado correctamente.")
            st.subheader("Resumen")
            st.dataframe(pd.DataFrame(resumen["control_resumen"]), use_container_width=True)

            c1, c2, c3 = st.columns(3)
            c1.metric("SKU unicos origen", resumen["sku_unicos_origen"])
